"""
POLAR-AI v2 — Live Antarctic Vessel Intelligence Platform
==========================================================
DATA_MODE=live by default.
Multi-vessel AIS tracking, real-data risk engine, global active vessel.
...
from app.routers import (
    ...
    system, satellite, alerts, live,
)
# New unified vessels router (replaces old vessel.py)
from app.routers import vessels as vessels_router
...
async def lifespan(app_instance: FastAPI):
    ...
    logger.info("=" * 60)
    logger.info(f"POLAR-AI v{settings.APP_VERSION} — LIVE VESSEL INTELLIGENCE")
    ...
    logger.info(f"  Sentinel-1 SAR:   {'✓ ' + settings.COPERNICUS_CLIENT_ID[:8] + '...' if settings.has_copernicus else '✗ (need COPERNICUS_CLIENT_ID/SECRET)'}")
    logger.info(f"  NSIDC Sea Ice:    ✓ free")
    logger.info(f"  NIC Icebergs:     ✓ free")
    logger.info(f"  Open-Meteo Wx:    ✓ free")
    logger.info(f"  Open-Meteo Ocean: ✓ free")
    logger.info(f"  AIS:              {'✓ ' + settings.AIS_PROVIDER if settings.has_ais else '✗ (need AIS_PROVIDER + AIS_API_KEY)'}")
    logger.info(f"  LLM:              {settings.llm_provider}")
    logger.info("=" * 60)

    # 1. Freshness registry
    ...
    if db_ok:
        ...
        logger.warning("✗ Database offline — continuing in limited mode")
    ...
    # 4. Wire WebSocket broadcast into services
    ...
    # Also wire into AIS manager for vessel position broadcasts
    from app.services.ais.ais_manager import get_registry as get_ais_registry
    get_ais_registry().set_broadcast_fn(live_manager.broadcast)

    # 5. Start AISStream listener (multi-vessel, full Antarctic bbox)
    if settings.has_ais and settings.AIS_PROVIDER.lower() == "aisstream":
        async def _start_aisstream():
            await asyncio.sleep(2)
            from app.services.ais.ais_manager import ensure_aisstream_running
            ensure_aisstream_running()
            logger.info("✓ AISStream multi-vessel listener started")
        asyncio.create_task(_start_aisstream())
    elif settings.has_ais:
        logger.info(f"✓ AIS REST polling configured: {settings.AIS_PROVIDER}")
...
app = FastAPI(
    title="POLAR-AI",
    description="""
## Antarctic Live Vessel Intelligence & Navigation Decision Support

**SIH26059** — Multi-vessel AIS tracking, real-data risk engine, satellite intelligence

### Data Modes
- **LIVE** (default) — real external data; OFFLINE when source unavailable
- **DEMO** — synthetic data for testing (`DATA_MODE=demo`)

> Research/decision-support prototype. Not a certified navigation system.
    """,
...
app.include_router(live.router,                prefix="/api",            tags=["Live Engine"])
app.include_router(system.router,              prefix="/api",            tags=["System"])
app.include_router(dashboard.router,           prefix="/api",            tags=["Dashboard"])
app.include_router(vessels_router.router,      prefix="/api",            tags=["Vessels"])
...
async def root():
    ...
    return {
        ...
        "vessels_tracked": reg.vessel_count(),
        "active_vessel_mmsi": reg.active_mmsi,
        "docs": "/api/docs",
        "live_status": "/api/live/status",
        "ws_live": "/ws/live",
        "ws_vessels": "/ws/vessels",
        "disclaimer": "Research decision-support prototype. Not for real navigation.",
    }
